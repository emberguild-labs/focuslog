...
**Hard Mode Focus Sessions**
- Optional toggle when starting a focus session that permanently disables the "End Session Early" button for the duration of the session
- Once active, the stop button is grayed out and clicking it shows a message instead of ending the session — it cannot be bypassed from within the extension
- The background service worker also enforces hard mode, rejecting any `STOP_FOCUS` messages while it is active
- The blocked page (`blocked.html`) displays a red warning banner explaining hard mode is active and reminding the user that uninstalling the extension is the only bypass — by design, to discourage it

**Double Confirmation Flow**
- Enabling hard mode triggers two sequential confirmation modals before the session starts, so it cannot be activated accidentally
- **Modal 1** — explains exactly what hard mode does, lists the restrictions, and warns that extension removal would bypass the block
- **Modal 2** — shows a summary card (duration, blocked domains, mode) and requires clicking "Lock It In" as a final deliberate commitment
- Both modals have a Cancel/Go Back option so the user can change their mind before confirming
