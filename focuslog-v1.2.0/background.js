...
async function enableFocusMode(durationMins, domains, hardMode = false) {
  const endsAt = Date.now() + durationMins * 60 * 1000;
  await chrome.storage.local.set({
    focusMode: { active: true, durationMins, endsAt, domains, hardMode }
  });
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === "START_FOCUS") {
    enableFocusMode(msg.durationMins, msg.domains, msg.hardMode || false)
      .then(() => sendResponse({ ok: true }));
    return true;
  }
  if (msg.type === "STOP_FOCUS") {
    // Block early stop if hard mode is active
    chrome.storage.local.get(["focusMode"], (r) => {
      const fm = r.focusMode;
      if (fm && fm.active && fm.hardMode) {
